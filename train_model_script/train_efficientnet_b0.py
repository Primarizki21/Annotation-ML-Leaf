"""Script 3 — EfficientNet-B0 (ImageNet pretrained, 2-phase FT, mandatory AMP).

What it does:
  Trains EfficientNet-B0 for binary classification (healthy vs
  needs_annotation) on 160x160 patches. 2-phase fine-tune: phase 1
  ...
  CosineAnnealingLR + mandatory AMP. Evaluates on the held-out test
  split, exports ONNX (fp32), and quantizes to dynamic int8.

How to run:
  uv run train_model_script/train_efficientnet_b0.py
  ...
  uv run train_model_script/train_efficientnet_b0.py --no_resume

  Periodic save controls:
  uv run train_model_script/train_efficientnet_b0.py --save_every 5 --max_checkpoints 3
...
GPU: REQUIRED (CUDA). The script raises RuntimeError if CUDA is
     unavailable — mandatory AMP per spec. First run downloads
     EfficientNet_B0 ImageNet weights (~20 MB, cached in
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train EfficientNet-B0 (ImageNet pretrained, 2-phase FT, mandatory AMP)",
        epilog=(
            "Examples:\n"
            "  uv run train_model_script/train_efficientnet_b0.py\n"
            "  uv run train_model_script/train_efficientnet_b0.py --epochs 10 --batch_size 16\n"
            "  uv run train_model_script/train_efficientnet_b0.py --no_resume"
        ),
...
def main() -> None:
    ...
    if not use_amp:
        raise RuntimeError("EfficientNet-B0 script requires CUDA (mandatory AMP per spec).")
    paths = make_output_dirs(args.output_dir, "efficientnet_b0")
    print_config(
        "efficientnet_b0", args, device, use_amp, paths,
    ...
    print(f"EfficientNet-B0 — trainable (init): {trainable_count(model):,} / total: {total_params:,}")
